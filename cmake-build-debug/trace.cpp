...
bool Sphere::intersect(const Ray &r, double t0, double t1, HitRecord &hr) const {
    ...
    double delta = sqr(dot(r.d,r.e-c))-dot(r.d,r.d)*(sqrMag(r.e-c)-sqr(rad));
    
    if (delta<0) return false;
    if (dot(r.d,r.d)<0.00001) return false;

    double t_min=(-dot(r.d,r.e-c)-sqrt(delta))/dot(r.d,r.d);
    double t_max=(-dot(r.d,r.e-c)+sqrt(delta))/dot(r.d,r.d);
    
    if(t_max<t0) return false;   //the ball is behind the viewer
    if(t_min>t1) return false;   //the ball is far
    if(t_min<t0) //viewer is inside the ball 
    {   
        if(t_max>t1) return false;   //the ball is huge
    ...
    // the ball is in front of the viewer
    hr.t=t_min;
    hr.p=r.e+hr.t*r.d;
    hr.n=hr.p-c;
    normalize(hr.n);
